...
Earnify takes the security of its software and the privacy of its users seriously. Because Earnify operates within the browser and executes compute-intensive tasks, maintaining trust through transparent and secure code is our top priority.
...
We recommend always using the latest version of the Earnify script.

---

## Security Architecture & Guarantees

Earnify is built with a strict security-first architecture:

- **Zero Data Collection:** Earnify does not access, store, or transmit any personally identifiable information (PII). It uses no cookies, no `localStorage`, and no fingerprinting.
...
- **No Dynamic Code Execution:** Earnify does not use `eval()`, `new Function()`, or dynamic `<script>` injection. It is fully Content Security Policy (CSP) compliant.
...
- **Direct Pool Connection:** Mining traffic goes directly from the user's browser to the configured mining pool via secure WebSocket (`wss://`). Earnify does not proxy mining traffic or intercept wallet addresses.
...
If you discover a security vulnerability within Earnify, please report it responsibly. We ask that you give us a reasonable amount of time to fix the issue before public disclosure.
...
The following issues are not considered security vulnerabilities in the context of Earnify:

- **CPU/GPU resource usage:** By design, Earnify uses CPU resources to mine cryptocurrency. This is its intended function. We provide throttling mechanisms (`autoMine(wallet, throttle)`) to limit this usage.
